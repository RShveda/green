...
function commonOnDocumentReady() {
    ...
    $('.btn-language').click(function () {
        if ($(this).data('language-link')) {
            ...
            window.location = $(this).data('language-link');
        }
    });
    ...
    $(window).load(function () {
        ...
        if (Cookies('SelectedAdvert') != null && Cookies('SelectedAdvert') !== '') {
            ...
            for (i = 0; i < selectedAdverts.length; i++) {
                ...
                $("span[data-advertid='" + selectedAdverts[i] + "']").addClass("icon-favourite-active");
            }
        } else {
            count_fav_text.hide();
        }
...
function languageSelectionOnDocumentReady() {
    if (window.localStorage && localStorage.getItem("languageLinks") != null) {
...
function homeMainOnDocumentReady() {
    ...
    if ($('body.ipad').length) {
        ...
        $('area.disable-area').data('maphilight', {
            fillColor: 'A3A3A3',
            fillOpacity: 1,
            alwaysOn: true
...
function searchAlertOnDocumentReady() {
    if ($('.no-result.no-area').length == 0) {
        ...
        else if (Cookies('show_mail_alert_dialog') != 'false' && Cookies('show_mail_alert_dialog') == 0) {
            if (Cookies('has_mail_alert') == undefined || Cookies('has_mail_alert') == null) {
                window.setTimeout(function () {
                    $("#createMailAlertDialog").modal('show');
                }, 2000);
            }
            Cookies.set('show_mail_alert_dialog', 'false', { expires: 365, path: '/' }); // set the cookie, with expiry after 1 day
        } else if (Cookies('show_mail_alert_dialog') != 'false') {
            Cookies.set('show_mail_alert_dialog', Cookies.get('show_mail_alert_dialog') - 1, { expires: 1, path: '/' });
        };
...
function autocompleteOnDocumentReady() {
    ...
    $('.city-autocomplete-wrapper input').keydown(function (e) {
...
function alignCitiesAutocomplete() {
    ...
    var width = $(".city-autocomplete-wrapper").width();
...
function favoritesClickOnDocumentReady(isAuthenticated) {
    var favoritesClickOnDocumentReadyHandler =
     function (e) {
         ...
         $("span[data-advertid='" + advertId + "']").toggleClass("icon-favourite-active");
         var selected = $(this).hasClass("icon-favourite-active") || $(this).hasClass("fav-active");
...
function putAdvertsToLocalStorage() {
    if (window.localStorage) {
        localStorage.clear();

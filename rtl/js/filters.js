...
RangeFilterInit = {
    RangeSlider: function (filterRangeInit, filterMax, filterW1, filterW2, filterValueMin, filterValueMax, filterRealValueMax, filterRealValueMin, filtersRange, order) {
        ...
        window.isInit = true;
        console.log("needs to be inited using RTL config");
        noUiSlider.create(filterRangeInit, {
            start: [filterW1, filterW2],
            /*we can set 'rtl' for everytime here without conditions because this js-file works only for RTL-version*/
            direction: 'rtl',
            behaviour: 'tap',
        ...
        console.log("was inited using RTL config");
...
function updateSearchLanguageURL(data) {
    ...
    if (searchQuery) {
        ...
        if ($(this).data('language-link') && isLocalStorageNameSupported()) {
            var languageLinks = {};
